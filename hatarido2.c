 ...
 * Adott Çv (h¢nap[nap]) napjaihoz adott napok d†tumai.
 * Munkanap †thelyezÇsek a "hatarido_20??.cfg" f†jlban "mm.dd-mm.dd" form†ban.
 * -m munkanappal kezd
 * -i interakt°v m¢d
 * -c rÇszben interakt°v
 * na nem munkanapra is eshet (nA u.a. -1 nap)
 * nb csak munkanapot sz†mol (nB u.a. -1 nap)
 * nC kezdã napot is sz†molja (-1 nap a vÇge)
 * nd 'jogerã'
 * RTF Çs CSV kimenetet gener†l Word Çs Excel (vagy LibreOffice) sz†m†ra.
 * kiemeles sargaval
 * TODO: UTF-8-ra √°t k√©ne m√©g √≠rni!
...
int is_munkanap(DT_DATE *dat)
{
  ...
  (void)DT_easter(husv);
  DT_add_days(husv, 1);  //hÇtfì!

  if (dt_eq(dat, husv) == 0)
    	hetvege = 1; //val¢j†ban h£svÇt!
  ...
  if (dt_eq(dat, punk) == 0)
    	hetvege = 1; //val¢j†ban pÅnkîsd!
       
  if (dt_eq(dat, nagyp) == 0)
    	hetvege = 1; //nagypÇntek!
...
int cal(int year, int month, FILE *f)
{
  ...
  char napok[] = "HÇ Ke Sz Cs PÇ Sz Va\n";
...
void id()
{
  puts("Hat†ridã sz†m°t¢ v."VERSION);
}
...
*/
  ...
  if (x > 2) 
  {
  	 ...
  	 if (e < get_year())
  	   	printf("\n*** Az Çvsz†m (%d) kisebb, mint a foly¢ Çv! ***", e);
  	 if (x == 99)
  	 {
  	   do {
  	   	 ...
  	   	 for (i=k; i<=x; i++)
  	   	 {
  	   	 	 ...
  	   	 	 if (r != -1)
  	   	 	 {
  	   	 	   ...
  	   	 	   fprintf(fr, R_E);
  	   	 	   	...
  	   	 	   	printf("%d.%d.%d. hÇtvÇge vagy Ånnep!\n", e,h,i);
  	   	 }
  	   	 printf("\n");
  	   	 fprintf(fr, "\n");
  	   	 if (h<m)
...
/*    puts("Hat†ridã sz†m°t¢ v."VERSION);*/
    id();
    puts("\t(munkanap †thelyezÇsek a 'hatarido_20??.cfg' f†jlban)");
#define ps "hatarido"
    printf("Haszn†lat:\t%s [-m] <ÇÇÇÇ[.hh[.nn]] n1 [n2 [...n10]]>\n", ps);
    printf("\tvagy:\t%s [-m] -i\n", s);
    printf("\tvagy:\t%s [-m] -c n1[a|A|b|B|C|d] [n2[a|A|b|B|C|d] ...]\n", s);
    puts("ahol:");
    puts("\t-m = munkanappal kezd");
    puts("\t-i = interakt°v m¢d");
    puts("\t-c = rÇszben interakt°v (csak d†tumot kÇr)");
    puts("\t#a = nem munkanapra is eshet");
    puts("\t#A = u.a. de a kezdã napot is sz†molja");
    puts("\t#b = csak munkanapot sz†mol");
    puts("\t#B = u.a. de a kezdã napot is sz†molja");
    puts("\t#C = a kezdã napot is sz†molja");
    puts("\t#d = 'jogerã'");
    return -1;
  }
